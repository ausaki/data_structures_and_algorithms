// title: word-ladder
// detail: https://leetcode.com/submissions/detail/192389026/
// datetime: Thu Nov 29 18:14:34 2018
// runtime: 220 ms
// memory: N/A
...
int queue_size(struct Queue *queue){
    return (queue->size + queue->tail - queue->head) % queue->size;
}

int queue_add(struct Queue *queue, void *node){
    if(queue_size(queue) == queue->size - 1){
        queue_expand(queue);
    }
